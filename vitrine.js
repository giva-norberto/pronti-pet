...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (logoPublico) {
            logoPublico.src = dados.logoUrl || "https://placehold.co/100x100/eef2ff/4f46e5?text=Pet";
        }

        const nomePublico = document.getElementById("nome-negocio-publico");
        if (nomePublico) {
            nomePublico.textContent = dados.nomeFantasia || "Pet Shop";
        }
        ...
        console.error("Erro fatal na inicialização:", error.stack || error);
...
async function aplicarPromocoesNaVitrine(listaServicos, empresaId, dataSelecionadaISO = null, forceNoPromo = false) {
    if (!empresaId || !Array.isArray(listaServicos)) return;
    ...
    listaServicos.forEach(servico => {
        ...
        if (!melhorPromocao) {
            melhorPromocao = promocoesAtivas.find(
                promo =>
                    ...
                    (Array.isArray(promo.servicoIds) && promo.servicoIds.length === 0)
            );
        }
        ...
        if (melhorPromocao) {
            ...
            if (precoAntigo <= 0) return;

            if (melhorPromocao.tipoDesconto === "percentual") {
                precoNovo = precoAntigo * (1 - Number(melhorPromocao.valor || 0) / 100);
            } else if (melhorPromocao.tipoDesconto === "valorFixo") {
                precoNovo = Math.max(precoAntigo - Number(melhorPromocao.valor || 0), 0);
            }
...
async function renderizarPlanosDeAssinatura(empresaId) {
    ...
    try {
        ...
        snapshot.forEach(docSnap => {
            ...
            const precoFormatado = Number(plano.preco || 0).toLocaleString('pt-BR', {
                style: 'currency',
                currency: 'BRL'
            });
            ...
            const servicosHTML = Array.isArray(plano.servicosInclusos)
                ? plano.servicosInclusos
                    .map(s => `<li>${s.quantidade}x ${s.nomeServico}</li>`)
                    .join('')
                : '';
            ...
            card.innerHTML = `
                <h3 style="color:#4f46e5;">${plano.nome || "Plano"}</h3>
                <p class="preco" style="color:#6366f1;font-weight:bold;font-size:1.2em;">
                    ${precoFormatado} / mês
                </p>
                <p>${plano.descricao || ''}</p>
                ...
                <button class="btn-assinar-plano" style="background:linear-gradient(90deg,#6366f1 0%,#4f46e5 100%);color:#fff;border:none;border-radius:8px;padding:8px 22px;margin-top:14px;font-size:1em;cursor:pointer;">
                    Assinar
                </button>
            `;
...
function handleUserAuthStateChange(user) {
    ...
    if (user && state.empresaId) {
        criarOuCompletarCliente(user);
        atualizarAssinaturasDoCliente();
...
async function criarOuCompletarCliente(user) {
    try {
        const refCliente = doc(db, "empresarios", state.empresaId, "clientes", user.uid);
...
async function atualizarAssinaturasDoCliente() {
    try {
        ...
        if (document.getElementById('lista-servicos')?.offsetParent !== null) {
            const servicosProfissional = state.todosOsServicos.filter(s =>
                state.agendamento?.profissional?.servicos?.includes(s.id)
            );

            UI.renderizarServicos(
                servicosProfissional,
                state.agendamento?.profissional?.horarios?.permitirAgendamentoMultiplo
            );
...
async function handleProfissionalClick(e) {
    ...
    if (!profissional) {
        await UI.mostrarAlerta("Erro", "Profissional não encontrado.");
        return;
    }
    ...
    try {
        ...
        const servicosDoProfissional = (profissional.servicos || [])
            .map(servicoId => state.todosOsServicos.find(servico => servico.id === servicoId))
            .filter(Boolean);
...
async function handleServicoClick(e) {
    ...
    const servicoSelecionado = {
        ...
        petNome: pet.nome,
        petPorte: pet.porte,
        preco: Number(precoDuracao.preco || 0),
        duracao: Number(precoDuracao.duracao || 0),
        precoCobrado: Number(precoDuracao.preco || 0)
    };
    ...
    if (permiteMultiplos) {
        ...
        const containerDataHorario = document.getElementById('data-e-horario-container');

        if (containerDataHorario) {
            containerDataHorario.style.display = 'block';
        }
...
async function handleProsseguirDataClick() {
    ...
    const container = document.getElementById('data-e-horario-container');
...
async function buscarPrimeiraDataDisponivel() {
    ...
    const duracaoTotal = state.agendamento.servicos.reduce(
        (total, s) => total + calcularDuracaoServico(s),
        0
    );

    try {
        const primeiraData = await encontrarPrimeiraDataComSlots(
            state.empresaId,
...
async function handleDataChange(e) {
    ...
    try {
        ...
        const agendamentosProfissional = todosAgendamentos.filter(
            ag => ag.profissionalId === profissional.id
        );
...
async function handleConfirmarAgendamento() {
    ...
    const { profissional, servicos, data, horario, pet } = state.agendamento;
    ...
    const textoOriginal = btn ? btn.textContent : "Confirmar Agendamento";
    ...
    try {
        const precoTotalCalculado = servicos.reduce(
            (total, s) => total + calcularPrecoServico(s),
            0
        );

        const duracaoTotalCalculada = servicos.reduce(
            (total, s) => total + calcularDuracaoServico(s),
            0
        );
...
async function handleCancelarClick(e) {
    ...
    const confirmou = await UI.mostrarConfirmacao(
    ...
    try {
        ...
        handleFiltroAgendamentos({
            target: document.querySelector('#botoes-agendamento .btn-toggle.ativo')
        });
...
async function entrarNaFilaDeAgendamento() {
    ...
    const pet = state.agendamento.pet || null;
    ...
    try {
        ...
        const servicosNormalizados = servicosSelecionados.map(s => ({
            id: s.id,
            nome: s.nome,
            duracao: calcularDuracaoServico(s),
            preco: calcularPrecoServico(s)
        }));
...
async function exigirCelularParaAgendamento(user) {
    ...
    const telefoneSalvo = String(perfil.telefone || "")
        .replace(/\D/g, "")
        .trim();
