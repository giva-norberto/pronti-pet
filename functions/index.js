...
// verificarEmpresa
// ============================================================================
exports.verificarEmpresa = onRequest({ region: REGION }, (req, res) => {
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      try {
        ...
        logger.warn("Erro ao buscar profissionais, assumindo 0.", {
...
exports.createPreference = onRequest({ region: REGION }, (req, res) => {
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      if (!token) {
        logger.warn("MERCADOPAGO_TOKEN ainda não configurado no Pronti Pet.");

        return res.status(503).json({
          error: "Pagamento temporariamente indisponível.",
          detalhes:
            "O Mercado Pago será ativado depois que o secret MERCADOPAGO_TOKEN for configurado.",
        });
      }
...
exports.receberWebhookMercadoPago = onRequest({ region: REGION }, (req, res) => {
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      if (!token) {
        logger.warn(
          "Webhook Mercado Pago recebido, mas MERCADOPAGO_TOKEN ainda não está configurado."
        );
      ...
      const paymentId =
        body?.data?.id || body.id || query["data.id"] || query.id || null;
...
exports.notificarDonoInstantaneo = onDocumentCreated(
  ...
  async (event) => {
    ...
    try {
      ...
      const nomeTutor =
        agendamento.clienteNome || agendamento.tutorNome || "Um tutor";

      const nomeServico = agendamento.servicoNome || "um serviço";
