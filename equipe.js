// ======================================================================
//                              EQUIPE.JS
//        VERSÃO FINAL, COMPLETA E REVISADA - PRONTI PET
// ======================================================================
...
function mapearElementos() {
    ...
    if (elementos.permitirAgendamentoMultiplo && elementos.capacidadePetsPorHorario) {
        elementos.permitirAgendamentoMultiplo.addEventListener('change', () => {
            sincronizarCapacidadePets(true);
        });
    }
}

/**
 * Controla o campo capacidade sem apagar valor salvo no carregamento.
 * usuarioAlterou = true somente quando o usuário muda o checkbox.
 */
function sincronizarCapacidadePets(usuarioAlterou = false) {
    if (!elementos.permitirAgendamentoMultiplo || !elementos.capacidadePetsPorHorario) return;

    const marcado = elementos.permitirAgendamentoMultiplo.checked;
    const input = elementos.capacidadePetsPorHorario;
    ...
    if (usuarioAlterou) {
        if (!marcado) {
            ...
            const valorAtual = parseInt(input.value, 10) || 1;
            if (valorAtual < 2) {
                input.value = 2;
            }
        }
...
function atualizarAreasAgendaEspecial() {
    ...
    if (elementos.agendaMesArea) {
        elementos.agendaMesArea.style.display = elementos.agendaTipo.value === "mes" ? "block" : "none";
    }

    if (elementos.agendaIntervaloArea) {
        elementos.agendaIntervaloArea.style.display = elementos.agendaTipo.value === "intervalo" ? "block" : "none";
    }
}
...
async function garantirPerfilDoDono() {
    ...
    try {
        ...
        if (!profissionalSnap.exists()) {
            ...
            const nomeDono = usuarioSnap.exists() && usuarioSnap.data().nome
...
async function iniciarListenerDaEquipe() {
    ...
    onSnapshot(query(profissionaisRef), (snapshot) => {
        renderizarEquipe(snapshot.docs.map(doc => ({
            id: doc.id,
            ...doc.data()
        })));
    }, (error) => {
...
function setupPerfilTabs() {
    ...
    if (elementos.agendaTipo) {
        elementos.agendaTipo.onchange = atualizarAreasAgendaEspecial;
    }
}
...
function renderizarEquipe(equipe) {
    ...
    if (equipe.length === 0) {
        elementos.listaProfissionaisPainel.innerHTML = `
            <div class="empty-state">
                <h3>👥 Equipe Vazia</h3>
                <p>Nenhum profissional na equipe ainda. Clique em "Convidar Funcionário" para começar.</p>
            </div>
        `;
        return;
    }
    ...
    equipe.sort((a, b) => {
        ...
        if (profissional.status === 'pendente') {
            botoesDeAcao = `
                <button class="btn btn-success" onclick="window.ativarFuncionario('${profissional.id}')">✅ Ativar</button>
                <button class="btn btn-danger" onclick="window.recusarFuncionario('${profissional.id}')">❌ Recusar</button>
            `;
        } else {
            botoesDeAcao = `
                <button class="btn btn-profile" onclick="window.abrirPerfilProfissional('${profissional.id}')">👤 Perfil</button>
        ...
        div.innerHTML = `
            <div class="profissional-foto">
                <img
                    src="${profissional.fotoUrl || "https://placehold.co/150x150/eef2ff/4f46e5?text=P"}"
                    alt="Foto de ${profissional.nome}"
                    onerror="this.src='https://placehold.co/150x150/eef2ff/4f46e5?text=P'"
                >
            </div>
            <div class="profissional-info">
                <span class="profissional-nome">${profissional.nome}</span>
                <span class="profissional-status">
                    ${profissional.status === 'pendente' ? 'Pendente de Ativação' : (profissional.ehDono ? 'Dono' : 'Funcionário')}
                </span>
            </div>
...
async function abrirPerfilProfissional(profissionalId) {
    try {
        ...
        if (elementos.modalPerfilProfissional) {
    ...
    try {
        ...
        if (horariosDoc.exists()) {
            ...
            if (elementos.permitirAgendamentoMultiplo) {
                elementos.permitirAgendamentoMultiplo.checked = !!horariosData.permitirAgendamentoMultiplo;
            }

            if (elementos.capacidadePetsPorHorario) {
                elementos.capacidadePetsPorHorario.value = Number(horariosData.capacidadePetsPorHorario || 1);
            }

            sincronizarCapacidadePets(false);
        } else {
            renderizarHorarios({
                ...horariosBase,
                intervalo: intervaloBase,
                permitirAgendamentoMultiplo: false,
...
function renderizarServicosNoPerfil(servicosSelecionados = []) {
    ...
    if (servicosDisponiveis.length === 0) {
        elementos.servicosLista.innerHTML = `
            <div class="servicos-empty-state">
                <p>Nenhum serviço cadastrado.</p>
                <p>Vá para a página de serviços para adicioná-los.</p>
            </div>
        `;
        return;
    }
...
function renderizarHorarios(horariosDataCompleta = {}) {
    ...
    diasSemana.forEach(dia => {
        const diaData = horariosDataCompleta[dia.key] || {
            ativo: false,
            blocos: [{ inicio: '09:00', fim: '18:00' }]
        };

        const estaAtivo = !!diaData.ativo;
        const blocos = diaData.blocos && diaData.blocos.length > 0
            ? diaData.blocos
...
function coletarHorarios() {
    ...
    document.querySelectorAll('.dia-horario').forEach(diaDiv => {
        ...
        const toggleDia = diaDiv.querySelector('.toggle-dia');
        const estaAtivo = toggleDia ? toggleDia.checked : false;
        ...
        horarios[dia] = {
            ativo: estaAtivo,
            blocos: blocos.length > 0 ? blocos : [{ inicio: '09:00', fim: '18:00' }]
        };
    });

    horarios.intervalo = elementos.inputIntervalo
        ? parseInt(elementos.inputIntervalo.value, 10) || intervaloBase
        : intervaloBase;

    horarios.permitirAgendamentoMultiplo = elementos.permitirAgendamentoMultiplo
        ? elementos.permitirAgendamentoMultiplo.checked
        : false;

    let capacidade = elementos.capacidadePetsPorHorario
        ? parseInt(elementos.capacidadePetsPorHorario.value, 10) || 1
        : 1;

    if (capacidade < 1) capacidade = 1;
    if (capacidade > 20) capacidade = 20;

    if (!horarios.permitirAgendamentoMultiplo) {
        capacidade = 1;
    }

    horarios.capacidadePetsPorHorario = capacidade;
...
function renderizarAgendaEspecial() {
    ...
    agendaEspecial.forEach((item, idx) => {
        let desc = (item.tipo === 'mes')
            ? `Mês: <b>${item.mes}</b>`
            : `De <b>${item.inicio}</b> até <b>${item.fim}</b>`;
        ...
        div.innerHTML = `
            <span>${desc}</span>
            <button type="button" class="btn btn-danger" data-agenda-idx="${idx}">Excluir</button>
        `;
...
async function adicionarAgendaEspecial() {
    ...
    if (tipo === 'mes') {
        if (!elementos.agendaMes.value) {
            return await showAlert("Aviso", "Selecione o mês.");
        }

        agendaEspecial.push({
            tipo: 'mes',
            mes: elementos.agendaMes.value
        });
    } else {
        if (!elementos.agendaInicio.value || !elementos.agendaFim.value) {
            return await showAlert("Aviso", "Informe o intervalo de datas.");
        }
        ...
        agendaEspecial.push({
            tipo: 'intervalo',
            inicio: elementos.agendaInicio.value,
            fim: elementos.agendaFim.value
        });
    }
...
async function salvarPerfilProfissional() {
    try {
        const servicosSelecionados = Array.from(
            document.querySelectorAll('.servico-item.selected')
        ).map(item => item.getAttribute('data-servico-id'));

        const horarios = coletarHorarios();

        let comissaoPadrao = elementos.comissaoPadrao && elementos.comissaoPadrao.value !== ""
            ? Number(elementos.comissaoPadrao.value)
            : null;
    ...
    try {
        ...
        if (totalAtivos >= limite) {
            await showAlert(
                ...
                `Seu plano permite até ${limite} profissionais.<br><br>Faça upgrade para ativar este funcionário.`
            );
...
async function recusarFuncionario(profissionalId) {
    const confirmado = await showCustomConfirm(
        "Recusar Funcionário",
        "Tem certeza que deseja recusar e excluir este cadastro pendente?"
    );
