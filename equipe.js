...
//        PRONTI PET: capacidade de pets por horário sem alterar lógica antiga
// ======================================================================
...
function mapearElementos() {
    ...
    // LÓGICA ORIGINAL DO PRONTI:
    // Permitir vários serviços de uma vez.
    ...
    // IMPORTANTE: estes campos precisam existir no HTML com estes IDs.
    elementos.permitirMultiplosPetsPorHorario = document.getElementById('permitir-multiplos-pets-horario');
    ...
    if (elementos.permitirMultiplosPetsPorHorario && elementos.capacidadePetsPorHorario) {
        elementos.permitirMultiplosPetsPorHorario.addEventListener('change', () => {
            sincronizarCapacidadePets(true);
        });
    }
}

// Controla somente a nova regra PET, sem mexer no permitirAgendamentoMultiplo antigo.
function sincronizarCapacidadePets(usuarioAlterou = false) {
    if (!elementos.permitirMultiplosPetsPorHorario || !elementos.capacidadePetsPorHorario) return;

    const permitirPets = elementos.permitirMultiplosPetsPorHorario.checked;
    elementos.capacidadePetsPorHorario.disabled = !permitirPets;
...
async function carregarDadosProfissional(profissionalId) {
    try {
        ...
        if (horariosDoc.exists()) {
            ...
            // Lógica original: vários serviços de uma vez
            ...
            // Nova lógica PET: vários pets no mesmo horário
            if (elementos.permitirMultiplosPetsPorHorario) {
                elementos.permitirMultiplosPetsPorHorario.checked = horariosData.permitirMultiplosPetsPorHorario || false;
            }

            if (elementos.capacidadePetsPorHorario) {
                elementos.capacidadePetsPorHorario.value = horariosData.capacidadePetsPorHorario || 1;
            }
            ...
            if (elementos.permitirMultiplosPetsPorHorario) {
                elementos.permitirMultiplosPetsPorHorario.checked = false;
            }
...
function coletarHorarios() {
    ...
    // Lógica original do Pronti: permitir vários serviços de uma vez
    if (elementos.permitirAgendamentoMultiplo) {
        horarios.permitirAgendamentoMultiplo = elementos.permitirAgendamentoMultiplo.checked;
    }

    // Nova lógica PET: permitir vários pets no mesmo horário
    if (elementos.permitirMultiplosPetsPorHorario) {
        horarios.permitirMultiplosPetsPorHorario = elementos.permitirMultiplosPetsPorHorario.checked;
        ...
        if (!horarios.permitirMultiplosPetsPorHorario) {
            capacidade = 1;
        }
